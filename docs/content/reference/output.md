...
describes what users get. Pick a format with `-o`, or let sinafinance choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sinafinance <command> -o table   # aligned columns for reading
sinafinance <command> -o jsonl   # one JSON object per line, for piping
sinafinance <command> -o json    # a single JSON array
sinafinance <command> -o csv     # spreadsheet friendly
sinafinance <command> -o tsv     # tab-separated
sinafinance <command> -o url     # just the URL column
sinafinance <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
sinafinance <command> --fields id,title,url
```
...
```bash
sinafinance <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sinafinance <command>            # a table, because this is a terminal
sinafinance <command> | wc -l    # JSONL, because this is a pipe
```
